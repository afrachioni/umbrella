...
void *manage (void *params) {
	...
	//setbuf (pipe, NULL);
	//mkfifo ("feed", S_IWUSR | S_IRUSR);
	//int fd = open ("feed", O_RDWR | O_NONBLOCK);// | O_CREAT);
	//fprintf (stdout, "Root has called open. fd: %d\n");
	...
	while(1) {
		...
		while (feof(pipe) || line[0] == '\0') {
			fprintf (stdout, "Root is sleeping on EOF...\n");
			//usleep(100000);
			sleep (1);
			clearerr (pipe);
			fgets (line, 500, pipe);
		}
		fprintf (stdout, "Just read from pipe: %s\n", line);
