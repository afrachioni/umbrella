...
int main(int narg, char **arg)
{
		...
		char file_paths[num_windows][MAX_FNAME_LENGTH];
		float targets[num_windows];
		float springs[num_windows];
		if (me == 0) {
			int j = 0;
		...
		MPI_Bcast (file_paths, num_windows * MAX_FNAME_LENGTH, MPI_CHAR, 0, \
				MPI_COMM_WORLD);
		MPI_Bcast (targets, num_windows, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast (springs, num_windows, MPI_FLOAT, 0, MPI_COMM_WORLD);

		char line[100];
		const int dump_freq = p->duration * DUMP_EVERY_STEPS;


		debugmsg ("Creating LAMMPSes...\n");
		//Setup LAMMPS instance with initial conditions and settings
		char *args[] = {(char*)"foo", (char*)"-screen", \
			(char*)"none", (char*)"-log", (char*)"none"};
		...
		double target_lbound = target_min - 0.5 * (target_max - target_min) / num_windows;
		double target_rbound = target_max + 0.5 * (target_max - target_min) / num_windows;
		printmsg ("\nIt might be useful to analyze these data by running WHAM "
				"something like this:\n");
		sprintf (line, "wham %f %f %d 0.1 %f 0 wham_metadata.txt wham_free.txt", \
				target_lbound, target_rbound, num_windows, p->temperature);
		printmsg ("%s\n", line);
		...
		double factor = -0.5 * springs[window_index]  / p->temperature;
